...
from app.api.models.docs import DocsResponse, DocumentPost, PageDocsResponse, HandleResponse
...
async def create_document(document: DocumentPost,
...
async def read_documents(
    ...
    documents = await docs_coll.find({"owner_id": user.id}).skip(skip).limit(page_size).to_list(page_size)
...
async def read_one_document(doc_id: str, user: User = Depends(current_active_user),
                            docs_coll: motor.motor_asyncio.AsyncIOMotorCollection = Depends(get_docs_coll)):

    doc = await docs_coll.find_one(
...
async def update_document(doc_id: str, document: DocumentPost, user: User = Depends(current_active_user),
                          docs_coll: motor.motor_asyncio.AsyncIOMotorCollection = Depends(get_docs_coll)):
...
async def delete_document(doc_id: str, user: User = Depends(current_active_user),
                          docs_coll: motor.motor_asyncio.AsyncIOMotorCollection = Depends(get_docs_coll)):

    delete_doc = await docs_coll.find_one_and_delete({"_id": doc_id, "owner_id": user.id})
    if not delete_doc:
        raise NotFound404Error
